...
(async function () {
  ...
  if (!storedPersonal || !storedEvent) {
    console.error('Missing stored personal or event data');
    window.location.href = '../registration.html';
    return;
  }

  let personalData, evData, munPortfolios, couponInfo;
  try {
    personalData  = JSON.parse(storedPersonal);
    evData        = JSON.parse(storedEvent);
    munPortfolios = storedMun ? JSON.parse(storedMun) : null;
    couponInfo    = storedCoupon ? JSON.parse(storedCoupon) : null;
  } catch (parseError) {
    console.error('Error parsing session data:', parseError);
    alert('Session data corrupted. Please start registration again.');
    window.location.href = '../registration.html';
    return;
  }

  console.log('✓ Session data parsed successfully');
  console.log('Event:', evData.event, '| Personal:', personalData.firstName, personalData.lastName);
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      console.log('✓ Registration data saved to Firebase');
        ...
        card.innerHTML = `
          <div style="text-align:center; padding:40px 0;">
            ...
            <h2 style="font-family:'Cormorant Garamond',serif;font-size:2rem;font-weight:300;margin-bottom:12px;">
              Registration <span style="color:#eab308;">Created!</span>
            </h2>
            <p style="font-family:'Outfit',sans-serif;color:#9ca3af;font-size:0.9rem;line-height:1.6;">
              Thank you, <strong style="color:#fff;">${registrationData.firstName}</strong>! Your registration has been <strong style="color:#eab308;">submitted for approval</strong>.<br>
              Transaction ID: <strong style="color:#fff;">${transactionId}</strong><br>
              We'll verify your payment and contact you via email once approved.
            </p>
      ...
      console.log('✓ Success message displayed. Redirecting in 3 seconds...');
      setTimeout(() => { 
